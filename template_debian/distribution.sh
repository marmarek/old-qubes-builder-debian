...
function aptUpgrade() {
    ...
    DEBIAN_FRONTEND="noninteractive" DEBIAN_PRIORITY="critical" DEBCONF_NOWARNINGS="yes" \
        chroot env APT_LISTCHANGES_FRONTEND=none eatmydata apt-get upgrade -u -y
}
...
function aptDistUpgrade() {
    aptUpdate
    DEBIAN_FRONTEND="noninteractive" DEBIAN_PRIORITY="critical" DEBCONF_NOWARNINGS="yes" \
        chroot env APT_LISTCHANGES_FRONTEND=none eatmydata apt-get dist-upgrade -u -y
}
...
function aptRemove() {
    files="$@"
    DEBIAN_FRONTEND="noninteractive" DEBIAN_PRIORITY="critical" DEBCONF_NOWARNINGS="yes" \
        chroot eatmydata apt-get ${APT_GET_OPTIONS} --force-yes remove ${files[@]}
}
...
function aptInstall() {
    files="$@"
    DEBIAN_FRONTEND="noninteractive" DEBIAN_PRIORITY="critical" DEBCONF_NOWARNINGS="yes" \
        chroot eatmydata apt-get ${APT_GET_OPTIONS} install ${files[@]}
    retcode=$?
